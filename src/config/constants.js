//CONSTANTES CONFIG

// mundo
export const canvasW = 400;
export const canvasH = 400;
export const numberOfHumans = 100;
export const graveSize = 0.25;
export const pixelesSize = 10;

export const normalHumanSize = 1;
export const normalHumanForce = 1;
export const normalHumanSpeed = 1;
export const normalHumanDamage = 1;
export const normalHumanKillRange = 1;

export const strongHumanSize = 1.25;
export const strongHumanSpeed = 1.8;
export const strongHumanDamage = 1.5;
export const minStrongHumanForce = 1.5;
export const maxStrongHumanForce = 2;
export const strongHumanKillRange = 1.5;

export const eliteHumanDamage = 2.5;
export const eliteHumanSize = 1.5;
export const eliteHumanSpeed = 3;
export const minEliteHumanForce = 2.5;
export const maxEliteHumanForce = 3.5;
export const eliteHumanKillRange = 2;

export const minGorillaSize = 2.5;
export const maxGorillaSize = 3;
export const minGorillaForce = 6;
export const maxGorillaForce = 10;
export const gorillaSpeed = 1.5;
export const gorrilaAceleration = 1;
export const gorillaDamage = 100;
export const gorillaKillRange = 1.5;
export const angleOfVision = 90; //REMEMBER THAT ITS MEANS 180 degrees
export const minGorillaforceToLaunch = 1.5;
export const maxGorillaforceToLaunch = 2.0;